...
object GlobalUtil {
    ...
    val appPackage: String
        get() = OpenEyeApplication.context.packageName
     ...
     */
    val appName: String
        get() = OpenEyeApplication.context.resources.getString(OpenEyeApplication.context.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = OpenEyeApplication.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            OpenEyeApplication.context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            OpenEyeApplication.context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(OpenEyeApplication.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
    ...
    fun getDimension(resId: Int): Int {
        return OpenEyeApplication.context.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return OpenEyeApplication.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = OpenEyeApplication.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            OpenEyeApplication.context.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = OpenEyeApplication.context.packageManager
